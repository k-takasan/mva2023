...
echo "###############################"
bash tools/dist_train.sh  configs/mva2023_backbone/resnext101/centernet_resnext101_140e_coco.py $GPU_NUM
...
echo "###############################"
bash tools/dist_train.sh  configs/mva2023_backbone/resnext101/centernet_resnext101_140e_coco_finetune.py $GPU_NUM
...
echo "###############################"
CONFIG=configs/mva2023_backbone/resnext101/centernet_resnext101_140e_coco_sample_hard_negative.py
CHECKPOINT=work_dirs/centernet_resnext101_140e_coco_finetune/epoch_20.pth #
NNODES=${NNODES:-1}
...
python -m torch.distributed.launch \
 ...
 hard_neg_example_tools/test_hard_neg_example.py \
    ...
    --hard-negative-file work_dirs/centernet_resnext101_140e_coco_finetune/train_coco_hard_negative.json \
...
echo "###############################"
bash tools/dist_train.sh  configs/mva2023_backbone/resnext101/centernet_resnext101_140e_coco_hard_negative_training.py $GPU_NUM
...
echo "###############################"
bash tools/dist_test.sh configs/mva2023_backbone/resnext101/centernet_resnext101_140e_coco_inference.py work_dirs/centernet_resnext101_140e_coco_hard_negative_training/epoch_20.pth \ #
2 --format-only --eval-options jsonfile_prefix=results


_time=`date +%Y%m%d%H%M`
mkdir -p submit/${_time}
mv submit/results.bbox.json submit/${_time}
# SUBMIT_FILE=`echo ./submit/${_time}/results.json`
# SUBMIT_ZIP_FILE=`echo ${SUBMIT_FILE//results.json/submit.zip}`
cd submit/${_time}
mv results.bbox.json results.json
# zip $SUBMIT_ZIP_FILE $SUBMIT_FILE
zip submit.zip results.json